...
const int C3DFBS::_fieldReadSizes[_numberOfDataFields] = {u16_size, u16_size, u16_size, u16_size, u16_size, float_size, float_size, float_size, float_size, float_size, float_size, float_size};
...
C3DFBS::C3DFBS(comms_protocol_t mode, uint8_t inputArg, int interrupt, int nReset, C3DFBSCallback onStreamStarted, C3DFBSCallback onStreamStopped)
    : _mode(mode), _interrupt(interrupt), _nReset(nReset), _onStreamStarted(onStreamStarted), _onStreamStopped(onStreamStopped)
{
    if (mode == COMMS_I2C)
...
bool C3DFBS::isAlive()
{
    ...
    return value == sensor_response_ok; // The expected return value
}
...
C3DFBS::status_t C3DFBS::startDataStream()
{
    ...
    if (status == SUCCESS)
    {
        if (value != sensor_response_ok) // The expected return value
...
void C3DFBS::printData(DataPacket *data, bool isStream, Print *dest) const
{
    if (isStream)
    {
...
{
    uint8_t value = sensor_response_unknown; // Default
    auto ret = read(Command::BIAS_SENSOR, &value);

    if (ret != SUCCESS)
        return ret;

    if (value == sensor_response_ok)
        return ret; // Success. Sensor biased

    if (value == sensor_response_error)
...
C3DFBS::status_t C3DFBS::setCommsMode(uint8_t protocol)
{
    ...
    uint8_t new_protocol = sensor_response_unknown; // Initialise to something invalid
...
int C3DFBS::getDataStreamSize() const
{
